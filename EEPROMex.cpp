...
uint64_t EEPROMClassEx::readLongLong(int address)
{
	uint32_t byteHigh, byteLow;
	uint64_t value = 0;
	
	if (!isReadOk(address+sizeof(uint32_t))) return 0;
	if (!isReadOk(address+sizeof(uint32_t)+sizeof(uint32_t))) return 0;
	byteHigh = eeprom_read_dword((unsigned long *) address);
	byteLow = eeprom_read_dword((unsigned long *) address + sizeof(uint32_t));
	
	value = ((uint64_t)byteHigh) << 32 | byteLow;
...
bool EEPROMClassEx::writeLongLong(int address, uint64_t value)
{
	uint32_t highValue, lowValue;
	
	highValue = (uint32_t)(value >> 32);
	lowValue = (uint32_t)value;
	
	if (!isWriteOk(address+sizeof(uint32_t))) return false;
	if (!isWriteOk(address+sizeof(uint32_t)+sizeof(uint32_t))) return false;
	eeprom_write_dword((unsigned long *) address, highValue);
	eeprom_write_dword((unsigned long *) address+sizeof(uint32_t), lowValue);
	return true;
}
